...
const Button = styled.button`
    ...
    ${(p) => (p.np ? `bprder: 1px solid ${Colors.GrayBorder}` : "")};
    padding: 0.5rem ${(p) => (p.np ? `1rem`: `3rem`)};
    font-size: 1.1rem;
    ${(p) => (p.fw ? "width: 100%" : "")};
    ${(p) => (p.disabled ? `opacity: 0.5; cursor: default;` : "")}
    ${(p) => (p.flex ? `display: flex; justify-content: center; align-content: center; column-gap: .4rem` : "")};
`;
